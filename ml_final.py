...
def generateDataframes(json_input):
    ...
    for movie in movies:
        ...
        movies[movie]["user-percentile-25"] = round(np.percentile(user_reviews, 25), 2)
        movies[movie]["user-percentile-75"] = round(np.percentile(user_reviews, 75), 2)
...
def generateTrainAndTest(opt):
    ...
    with (
        open("data/" + train_filename, "r") as json_train,
        open("data/" + test_filename) as json_test,
    ):
...
def naiveBayes(opt):
    ...
    proba_y = pd.DataFrame(index=x_test.index.values, data=proba, columns=labels)
