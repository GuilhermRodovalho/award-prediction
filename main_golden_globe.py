...
def get_reviews(url, release_year, cerimony_date, critic=False):
    ...
    for review in reviews:
        ...
        if critic:  # As critic reviews always ocurr days after the movie launched
            ...
            review_date_obj = datetime.strptime(review_date_str, "%b %d, %Y").date()
            if cerimony_date > review_date_obj.strftime("%d/%m/%Y"):
                reviews_list.append(review_score)
    try:
        next_page_flipper = soup.find("div", class_="page_flipper")
        if next_page_flipper:
            next_page = next_page_flipper.find("span", class_="flipper next").find("a")
            if next_page:
                next_page_url = "https://www.metacritic.com" + next_page["href"]
                reviews_list.extend(
                    get_reviews(next_page_url, release_year, cerimony_date, critic)
                )
        return reviews_list
...
def processaArquivoUserReview(film, film_year, oscar_date):
    ...
    try:
        return get_reviews(
            f"https://www.metacritic.com/movie/{film_string}/user-reviews?sort-by=date&num_items=100",
            film_year,
            oscar_date,
        )
    except Exception as e:
...
def processaArquivoCriticReview(film, film_year, oscar_date):
    ...
    try:
        return get_reviews(
            f"https://www.metacritic.com/movie/{film_string}/critic-reviews?sort-by=date&num_items=100",
            film_year,
            oscar_date,
            True,
        )
...
if __name__ == "__main__":
    with (
        open("csv/golden_globe_comedy_2023.csv") as csv_file,
        open("data/golden_globe_movies_comedy_2023_data.json", "w") as movies_file,
    ):
        movies_dict = dict()
        ...
        try:
            for row in csv_reader:
                if line_count == 0:
                    ...
                    critic = processaArquivoCriticReview(row[2], row[0], row[1])
                    movies_dict[row[2]] = {
                        "user-review": user,
